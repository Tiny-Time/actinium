<x-guest-layout>
    @section('title', __('Reset Password - TinyTime'))

    @section('description', __('Reset your TinyTime account password securely. Regain access to effortless event
        planning and personalized event management.'))

        <x-authentication-card title="Reset Password">
            <x-slot name="logo">
            ...
            <form method="POST" action="{{ route('password.update') }}" class="relative">
                ...
                @php
                    function maskEmail($email)
                    {
                        ...
                        $maskedUsername =
                            substr($username, 0, 2) . str_repeat('*', strlen($username) - 4) . substr($username, -2);
