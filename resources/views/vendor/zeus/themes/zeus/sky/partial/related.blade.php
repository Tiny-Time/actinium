<div class="flex sm:space-x-2 rtl:space-x-reverse px-2 lg:p-4 w-full">
    <a class="mb-4 md:mb-0 w-full relative h-[16em] sm:h-[20em] md:h-[22em] lg:h-[24em]" href="{{ route('post',$post->slug) }}">
        ...
        @if($post->image() !== null)
            <img loading="lazy" alt="{{ $post->title }}" src="{{ $post->image() }}" class="absolute ltr:left-0 rtl:right-0 top-0 w-full h-full shadow-md rounded-[2rem] @if($loop->first) md:ltr:rounded-br-none md:rtl:rounded-bl-none @else md:ltr:rounded-bl-none md:rtl:rounded-br-none @endif z-0 object-cover"/>
        @endif
        ...
        <div class="p-4 absolute bottom-0 ltr:left-0 rtl:right-0 z-20">
            ...
            <div class="flex mt-3">
                <img loading="lazy" src="{{ \Filament\Facades\Filament::getUserAvatarUrl($post->author) }}" class="h-10 w-10 rounded-full mx-2 object-cover"/>
                <div>
