    ...
    <div class="w-full px-4 mx-auto mt-6 max-w-7xl sm:px-6 lg:px-8">
        ...
        <div class="grid gap-6 mt-8 md:grid-cols-3">
            ...
            <article class="md:col-span-2">
                ...
                <div class="flex flex-wrap justify-center gap-3 mt-3 text-xs font-semibold uppercase">
                    ...
                    @unless ($post->tags->isEmpty())
                        @if (!empty($post->tags->where('type', 'tag')->first()))
                ...
                <div class="flex flex-wrap gap-3 items-center justify-center mt-3 text-xs font-semibold uppercase">
                    @unless ($post->tags->isEmpty())
                        @if (!empty($post->tags->where('type', 'category')->first()))
                ...
                {{-- Post Share buttons --}}
                <div class="flex flex-wrap items-center justify-center gap-2 mt-3 text-sm font-semibold uppercase">
                    <span>Share: </span>
                    <a href="https://www.facebook.com/sharer.php?u={{ route('post', $post->slug) }}"
                        class="flex items-center gap-1 px-2 py-1 bg-blue-600 rounded shadow hover:bg-olivine text-gray-50">
                    ...
                    <a href="https://twitter.com/intent/tweet?text={{ $post->title ?? '' }}&url={{ route('post', $post->slug) }}"
                        class="flex items-center gap-1 px-2 py-1 bg-black rounded shadow hover:bg-olivine text-gray-50">
                    ...
                    <a href="https://pinterest.com/pin/create/button/?url={{ route('post', $post->slug) }}&media={{ !empty($post->image()) ? $post->image() : Vite::asset('resources/images/bg.jpg') }}&description={{ $post->title ?? '' }}"
                        class="flex items-center gap-1 px-2 py-1 bg-red-600 rounded shadow hover:bg-olivine text-gray-50">
            ...
            @include($skyTheme . '.partial.sidebar', [
                'tags' => \LaraZeus\Sky\Models\Tag::where('type', 'category')->get(),
                'recent' => \LaraZeus\Sky\Models\BlogPost::latest()->take(5)->get(),
            ])
        </div>
    </div>
