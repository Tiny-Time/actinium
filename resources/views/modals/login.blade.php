<div class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true" x-show="openLoginModal" x-cloak>
    <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" x-show="openLoginModal"
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex items-center justify-center min-h-full p-4">
            <div class="relative w-full max-w-xs overflow-hidden transition-all transform bg-gray-100 rounded shadow-xl lg:max-w-3xl"
                x-show="openLoginModal" x-transition:enter="ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
                x-transition:leave="ease-in duration-200"
                x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                @click.away="openLoginModal = !openLoginModal">
                <div class="grid bg-gray-100 lg:grid-cols-2 dark:bg-gray-900">
                    <div class="flex" x-data="{ showLogin: false }">
                        <!-- Social Login -->
                        <div class="relative flex flex-col items-center justify-center w-full py-6 transition-all"
                            x-show="!showLogin" x-transition:enter="transform duration-300 ease-out"
                            ...
                            <div class="w-full max-w-sm px-6 py-4 mt-2">
                                ...
                                @livewire('modals.social-signin-form')
                            </div>
                        </div>
                        <!-- Email Login -->
                        <div class="relative flex flex-col items-center justify-center w-full py-6" x-show="showLogin"
                    ...
                    <div
                        ...
                        <svg class="absolute w-5 h-5 cursor-pointer right-2 top-2"
                            @click="openLoginModal = !openLoginModal" viewBox="0 0 30 30" fill="none"
