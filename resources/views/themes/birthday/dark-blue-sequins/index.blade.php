<x-guest-layout>
    @push('css')
        <link rel="stylesheet" href="{{ Vite::asset('resources/views/themes/birthday/dark-blue-sequins/css/style.css') }}" />
    @endpush
    @push('js')
    ...
    <!-- Main timer template -->
    <section class="flex flex-col items-center gap-5 p-4 -mb-6 text-white toz toz-main">
        ...
        <div class="flex justify-center gap-2">
            @if ($event->user_id)
                <button type="button" @click="$store.openCreateRSVPModal.toggle()"
                    class="hover:text-blue-600 px-4 py-2 uppercase bg-[#11a6a7] rounded-3xl text-sm text-[#32214d] font-bold">rsvp</button>
            @else
                <button type="button" @click="$store.openSubscriptionModal.toggle()"
                    class="hover:text-blue-600 px-4 py-2 uppercase bg-[#11a6a7] rounded-3xl text-sm text-[#32214d] font-bold">rsvp</button>
            @endif
            <button type="button" @click="$store.openShareModal.toggle()"
                class="hover:text-blue-600 bg-[#11a6a7] p-2 rounded-full text-[#32214d]">
        ...
        <div class="p-4 mx-auto rounded-lg toz-subscription toz-bg-timer max-w-max">
            ...
            <form action="{{ route('tsubscribe') }}" method="post" id="tsubscribe"
                class="flex p-1 mx-auto mt-3 text-sm bg-white text-[#32214d] toz-form rounded-xl max-w-max overflow-clip md:text-base">
                ...
                <button type="submit"
                    class="toz-f-btn px-4 py-2 font-semibold bg-[#11a6a7] rounded-lg">Subscribe</button>
            </form>
            ...
            <div class="flex justify-center gap-2 mt-3 toz-sl">
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ route('homePage') }}"
                    ...
                    class="flex items-center justify-center bg-white text-[#091253] hover:text-blue-600 rounded-full toz-social-link w-10 h-10">
        ...
        <div class="mt-4 text-center">
            <a href="{{ config('app.url', 'TinyTime') }}"
                class="text-sm font-semibold hover:text-blue-600 md:text-xl">Powered by
                TinyTi.me</a>
        </div>
    </section>
